...
void Pseudo_8_sparse_mul(Fp12 *ANS,Fp12 *A,Fp12 *B){
    ...
    Fp2_add(&TMP2_FP12.x1.x1,&TMP3_FP2,&A->x1.x1);
    Fp2_add(&TMP1_FP2,&TMP1_FP2,&A->x1.x0);
    Fp2_mul(&TMP3_FP2,&A->x0.x2,&B->x1.x1);
    Fp2_mul_basis(&TMP3_FP2,&TMP3_FP2);
    Fp2_add(&TMP2_FP12.x1.x0,&TMP1_FP2,&TMP3_FP2);
    Fp2_add(&TMP1_FP2,&TMP2_FP2,&A->x1.x2);
    Fp2_mul(&TMP2_FP2,&A->x0.x2,&B->x1.x0);
    Fp2_add(&TMP2_FP12.x1.x2,&TMP1_FP2,&TMP2_FP2);
    ...
    Fp2_sub(&TMP3_FP2,&TMP3_FP2,&TMP1_FP2);
    Fp2_sub(&TMP3_FP2,&TMP3_FP2,&TMP2_FP2);
    Fp2_add(&TMP2_FP12.x0.x2,&TMP3_FP2,&A->x0.x2);
    Fp2_add(&TMP1_FP2,&TMP1_FP2,&A->x0.x1);
    Fp2_mul(&TMP3_FP2,&A->x1.x2,&B->x1.x1);
    Fp2_mul_basis(&TMP3_FP2,&TMP3_FP2);
    Fp2_add(&TMP2_FP12.x0.x1,&TMP1_FP2,&TMP3_FP2);
    Fp2_mul(&TMP1_FP2,&A->x1.x2,&B->x1.x0);
    Fp2_add(&TMP1_FP2,&TMP1_FP2,&TMP2_FP2);
    Fp2_mul_basis(&TMP1_FP2,&TMP1_FP2);
    Fp2_add(&TMP2_FP12.x0.x0,&TMP1_FP2,&A->x0.x0);
...
void BN12_print_parameters(){
    ...
    mpz_init(mod_12);
    printf("====================================================================================\n");
    printf("BN12 Class 2 (HW=6)\n\n");
    
    gmp_printf("Parameters\n");
    gmp_printf("X = 2^{114}+2^{94}+2^{55}-2^{53}-2^{3}-1\n");
